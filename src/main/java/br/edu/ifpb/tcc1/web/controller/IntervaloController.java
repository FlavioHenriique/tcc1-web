...
public class IntervaloController {
    ...
    public Grafico buscaProgramaPorAno(int ano1, int ano2, String funcao, String subfuncao) {
        Grafico grafico = new Grafico("Programas da subfunção " + subfuncao
                + " entre " + ano1 + " e " + ano2);
        grafico.setType("pie");
        grafico.setData(query.programaPorAno(ano1, ano2, funcao, subfuncao));
        grafico.setName("Empenhos");
        grafico.setCategorias(categorias(grafico.getData()));
        return grafico;
    }
    ...
    public Grafico buscaProgramaPorSemestre(int semestre, String funcao, String subfuncao) {
        String sem = "" + semestre;
        Grafico grafico = new Grafico(("Programas da subfunção " + subfuncao + " no "
                + sem.substring(0, 1)
                + "º semestre de "
                + sem.substring(1, sem.length())));
        grafico.setData(query.programaPorSemestre(semestre, funcao, subfuncao));
        grafico.setType("pie");
        grafico.setName("Empenhos");
        grafico.setCategorias(categorias(grafico.getData()));
        return grafico;
    ...
    public Grafico buscaProgramaPorMes(int mes1, int mes2, String funcao, String subfuncao) {
        Grafico grafico = new Grafico("Programas da subfunção " + subfuncao + " entre meses");
        grafico.setType("pie");
        grafico.setData(query.programaPorMes(mes1, mes2, funcao, subfuncao));
        grafico.setName("Empenhos");
        grafico.setCategorias(categorias(grafico.getData()));
        return grafico;
    }
    ...
    public Grafico buscaAcaoPorAno(int ano1, int ano2, String funcao, String subfuncao, String programa) {
        Grafico grafico = new Grafico("Ações do programa " + programa
                + " entre " + ano1 + " e " + ano2);
        grafico.setType("pie");
        grafico.setData(query.acaoPorAno(ano1, ano2, funcao, subfuncao, programa));
        grafico.setName("Empenhos");
        grafico.setCategorias(categorias(grafico.getData()));
        return grafico;
    }
